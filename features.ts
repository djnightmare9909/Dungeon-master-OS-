...
export async function updateLogbookData(type: 'sheet' | 'inventory' | 'quests' | 'npcs' | 'achievements') {
  ...
  try {
    ...
    const response = await ai.models.generateContent({ model: getUISettings().activeModel, contents: prompt });
...
export async function generateCharacterImage() {
  ...
  try {
    ...
    const descriptionResponse = await ai.models.generateContent({ model: getUISettings().activeModel, contents: descriptionPrompt });
